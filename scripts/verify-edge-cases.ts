...
async function run() {
  console.log('====================================================');
  console.log('LegalLens AI — Edge Cases & Adversarial Verification');
  console.log('====================================================\n');
  ...
  const testEmail = `edge_user_${Date.now()}@legallens.internal`;
  ...
  try {
    ...
    if (page) {
      ...
      await page.evaluate((tok) => {
        localStorage.setItem('legallens_auth_token', tok);
      }, token);
      await page.reload();
  ...
  try {
    ...
    const unicodeNotes =
      ...
      'LegalLens analysis fidelity verification test. '.repeat(100);
