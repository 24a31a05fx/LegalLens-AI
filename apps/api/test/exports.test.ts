...
import { sessionStore } from '@legallens/security';
...
describe('Phase 8: Export & Data Controls (06_Implementation_Plan.md §10)', () => {
  ...
  beforeEach(async () => {
    ...
    const resA = await request(app)
      ...
      .send({ email: 'client@legallens.internal', password: 'Password123!', displayName: 'Client A' });
...
`;
    ...
    const resB = await request(app)
      .post('/api/v1/auth/signup')
      .send({ email: 'intruder@legallens.internal', password: 'Password123!', displayName: 'Intruder' });
  ...
  describe('Export Generation and Invariants', () => {
    it('generates a Markdown export with mandatory disclaimer, timestamp, and source references', async () => {
      ...
      expect(text).toContain('LegalLens AI provides automated informational analysis');
