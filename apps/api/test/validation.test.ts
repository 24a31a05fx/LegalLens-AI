...
describe('Strict Input Validation Middleware (Zod)', () => {
  ...
  beforeEach(async () => {
    ...
    await request(app)
      ...
      .send({ email: 'valid@legallens.ai', password: 'ValidPassword123!' });
  });

  it('rejects requests with missing required fields with HTTP 400', async () => {
    const res = await request(app)
      .post('/api/v1/auth/login')
      .send({ email: 'valid@legallens.ai' }); // missing password
  ...
  it('rejects passwords shorter than schema length with HTTP 400', async () => {
    const res = await request(app)
      .post('/api/v1/auth/login')
      .send({ email: 'valid@legallens.ai', password: 'short' }); // < 8 chars
