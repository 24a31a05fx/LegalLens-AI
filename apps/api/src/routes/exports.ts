...
import { createNotFoundOrForbiddenError, createValidationError } from '@legallens/security';
import { ExportFormatSchema, ExportSourceTypeSchema } from '@legallens/shared-types';
import { z } from 'zod';
...
exportsRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const filename = `LegalLens_${exp.export_source_type}_${exportId.substring(0, 8)}.${exp.format}`;
