...
import {
  ...
  Confidence,
  DocumentChunk,
} from '@legallens/shared-types';
import { defaultLogger } from '@legallens/logger';
  ...
  for (const chunk of versionChunks) {
    ...
    if (params.analysisType === 'summary' || params.analysisType === 'clauses') {
      if (textLower.includes('rent') || textLower.includes('payment') || textLower.includes('fee')) {
        rawFindings.push({
          finding_type: 'fee',
          title: 'Payment & Fee Provisions',
          explanation:
            readingLevel === 'simple'
              ? 'Plain summary: The document outlines recurring fee obligations and payment due dates.'
              : `Detailed legal provision: Payment obligations, rate calculation, and default interest penalties: ${chunk.text_content.substring(0, 150)}...`,
          confidence: 'high',
          chunk_ids: [chunk.id],
        });
      }

      if (textLower.includes('terminat') || textLower.includes('default') || textLower.includes('notice')) {
        rawFindings.push({
          finding_type: 'termination',
          title: 'Termination & Default Clause',
          explanation:
            readingLevel === 'simple'
              ? 'Plain summary: The agreement allows termination under specific written notice rules.'
              : `Detailed legal provision: Termination covenants, notice windows, and cure period stipulations: ${chunk.text_content.substring(0, 150)}...`,
          confidence: 'high',
          chunk_ids: [chunk.id],
        });
      }

      if (textLower.includes('law') || textLower.includes('dispute') || textLower.includes('jurisdiction')) {
        rawFindings.push({
          finding_type: 'dispute',
          title: 'Governing Law and Dispute Resolution',
          explanation:
            readingLevel === 'simple'
              ? 'Plain summary: Disputes will be decided under the specified state jurisdiction.'
              : `Detailed legal provision: Choice of forum, conflict-of-laws principles, and dispute venue provisions: ${chunk.text_content.substring(0, 150)}...`,
          confidence: 'high',
          chunk_ids: [chunk.id],
        });
      }
    ...
    if (params.analysisType === 'review_points') {
      ...
      if (
        ...
        rawFindings.push({
          ...
          title: 'Review Point: Notable Clause Requiring Attention',
          explanation:
            readingLevel === 'simple'
              ? 'Plain summary: Provision contains clauses that limit standard remedies or grant unilateral discretion.'
              : `Detailed legal provision: Section requires careful review regarding mutual rights: ${chunk.text_content.substring(0, 150)}...`,
          confidence: 'high',
          chunk_ids: [chunk.id],
        });
      }
